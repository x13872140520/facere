...
export default {
  ...
  mounted(){
    ...
    var that =this
    //1111111111111111
       axios.get('https://www.yun61.com/member/index.php?m=face_sign&ac=get_token').then(function(res){
         console.log('res');
          console.log(res);
         if(res.status==200){
   ...
   methods:{
     ...
     async setCaptureData(){
      ...
      try{
        ...
        //this.faceMatcher 相当于人脸库资料.
          this.faceMatcher = new faceapi.FaceMatcher(this.results)
          //传单人的采集信息至后台
    ...
    console.log(this.searchResult.tname+','+this.searchResult.stu_id);
     ...
     async  run() {
      ...
      // to the video element
       await faceapi.nets.ssdMobilenetv1.loadFromUri('/member2/face/models')
    ...
    pro.then( async function(){
      ...
      that.results = await faceapi.detectAllFaces(that.imgEl, new faceapi.SsdMobilenetv1Options({  minConfidence:0.7 })).withFaceLandmarks().withFaceDescriptors()
       console.log(that.results);//多脸的情况下是Array
       
      if(that.faceMatcher){
        for(var i=0;i<that.results.length;i++){
          const bestMatch = that.faceMatcher.matchDescriptor(that.results[i].descriptor)
         console.log('匹配度');
         console.log(that.faceMatcher);
      ...
      if (that.results) {
        ...
        faceapi.draw.drawDetections(canvas, faceapi.resizeResults(that.results, dims))
        that.initStatu=true

        if(that.isFirstGetFacePool){
          that.faceMatcher = new faceapi.FaceMatcher(that.results)
          that.getFacePool()
  ...
  watch:{
    searchResult(newValue){
      console.log('searchResultsearchResult');
